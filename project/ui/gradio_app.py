...
def create_gradio_ui():
    ...
    dir_path = Path(config.MARKDOWN_DIR)#pdf文件
    for file in dir_path.iterdir():
        if file.is_file():
            print(file.name)
            have_files.append(file)
    doc_manager.add_documents(have_files)

    
    def format_file_list():
        files = doc_manager.get_markdown_files()
        print("123files=",files)#正常显示
        if not files:
    ...
    with gr.Blocks(title="Agentic RAG") as demo:
        ...
        with gr.Tab("Documents", elem_id="doc-management-tab"):
            ...
            files_input = gr.File(
                ...
                file_count="multiple",
                type="filepath",#将上传的文件保存为临时文件路径（而不是二进制内容），后端可直接通过路径读取。
                height=200,
                show_label=False
            )
