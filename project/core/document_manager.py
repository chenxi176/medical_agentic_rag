...
class DocumentManager:
    ...
    def __init__(self, rag_system):
        ...
        # print("123",self.markdown_dir)
        #agentic-rag-for-dummies-main/markdown_docs
    ...
    def get_markdown_files(self):
        # print('test',self.markdown_dir.exists()) #true
        if not self.markdown_dir.exists():
            return []

        files = sorted([p.name.replace(".md", ".pdf") for p in self.markdown_dir.glob("*.md")])
        # files.extend([p.name for p in self.markdown_dir.glob("*.pdf")])
        return files  # 如果最终也要排序
